...
public class LowerLeftCornerWall extends WallPattern {
    ...
    public void render(SpriteBatch batch, Coordinate position) {
        batch.draw(textureMap.get(Textures.WALL_CORNER_FRONT_LEFT), position.getX() * textureMap.get(Textures.WALL_CORNER_FRONT_LEFT).getWidth(), position.getY() * textureMap.get(Textures.WALL_CORNER_FRONT_LEFT).getHeight());
